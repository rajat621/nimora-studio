...
// GSAP is dynamically imported inside the effect so it's not included in the SSR bundle
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        ctx = Gmod.context(() => {
          const allLines = Gmod.utils.toArray<HTMLElement>(`.${styles.heroContainer} .${styles.revealInner}`);
          const heroLines = allLines.slice(0, -1);
          const descLine = allLines[allLines.length - 1];

          Gmod.set(allLines, { y: "100%" });
        ...
        console.error("GSAP failed to load for About page:", err);
      }
    })();
